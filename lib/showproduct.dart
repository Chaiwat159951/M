...
// Stateful Widget for ShowProduct
...
class _MyHomePageState extends State<ShowProduct> {
        ...
        return AlertDialog(
          ...
          actions: [
            ...
            TextButton(
              onPressed: () {
                Navigator.of(dialogContext).pop(); // Close the dialog
                deleteProduct(key, context); // Call deleteProduct to remove data
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
    ...
    try {
      ...
      if (snapshot.exists) {
        ...
        snapshot.children.forEach((child) {
          ...
          product['key'] = child.key; // Store key for reference (like edit/delete)
          loadedProducts.add(product);
        });

        loadedProducts.sort((a, b) => b['price'].compareTo(a['price'])); // Sort by price (descending)

        setState(() {
          products = loadedProducts; // Update the UI with the fetched products
        });
  ...
  void initState() {
    super.initState();
    fetchProducts(); // Fetch products when the widget is created
  }
